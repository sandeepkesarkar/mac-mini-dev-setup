---
name: fanout
description: Run independent subtasks in parallel — one git worktree and one implementation sub-agent per task, each opening its own PR — then cross-review every PR. The orchestrator never merges; the human does.
---
...
1. Per task, create an isolated worktree:
   `sys_os_shell("git worktree add .worktrees/<task_id> -b <bundle>/<task_id>")`.
   Record the worktree path + branch in the registry
   (`.polly/registry.json`, or whatever task ledger this bundle uses).
...
5. The orchestrator does NOT merge — the PR is the deliverable. When
   cross-review passes, the task is done: mark it ready in the registry with
   its PR URL and leave it for the human to review and merge. Never run
...
- Because the orchestrator never merges, cross-PR conflicts surface when the
  human merges, not here. Keeping each parallel task's file scope disjoint is
  what keeps that rare — honor it.
